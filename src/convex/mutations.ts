...
export function convexInsert(props: ConvexInsertProps) {
  ...
  return mutationGeneric({
    ...
    handler: async (ctx, args) => {
      // Filter out read-only Convex system fields that cannot be inserted
      const insertableData: Record<string, any> = {};
      for (const [key, value] of Object.entries(args.data)) {
        // Skip Convex system fields (_id, _creationTime) - they are auto-generated
...
export function convexUpsert(props: ConvexUpsertProps) {
  ...
  return mutationGeneric({
    ...
    handler: async (ctx, args) => {
      // Filter out read-only Convex system fields
      const upsertableData: Record<string, any> = {};
      for (const [key, value] of Object.entries(args.data)) {
        // Skip Convex system fields (_id, _creationTime) - auto-generated on insert, read-only on patch
        if (key === "_id" || key === "_creationTime") {
          continue;
        }
      ...
      if (args.id) {
        ...
        if (existing) {
          await ctx.db.patch(args.id as any, upsertableData);
          docId = args.id;
          wasUpdate = true;
        } else {
          // args.collection is already prefixed by adapter
          docId = await ctx.db.insert(args.collection as any, upsertableData);
        }
      } else {
        // args.collection is already prefixed by adapter
        docId = await ctx.db.insert(args.collection as any, upsertableData);
...
export async function adapterUpsert<T>(props: AdapterUpsertProps<T>) {
  ...
  // Filter out read-only Convex system fields
  // For upsert: remove _id and _creationTime (for inserts), but keep other fields for patches
  const upsertableData: Record<string, any> = {};
  for (const [key, value] of Object.entries(compiledData)) {
    // Skip Convex system fields (_id, _creationTime) - auto-generated on insert, read-only on patch
    if (key === "_id" || key === "_creationTime") {
